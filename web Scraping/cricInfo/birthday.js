...
function parseHtml(html) {
    ...
    for (let i = 0; i < bothInningHtml.length; i++) {
        ...
        if (WinningTeamName == teamName) {
            ...
            // -> jo rows belong karti kisko -> batsman 
            // loop -> tr batsman -> use 
            for (let i = 0; i < rows.length; i++) {
                ...
                if (tdlength > 4) {
                    ...
                    let batsmanRow = rows[i];
                    let tds = batsmanRow.querySelectorAll("td");
    ...
    function parseBirthdayHtml(html,name){
        let dom = new JSDOM(html);
        let MyDocument = dom.window.document;
        let recievedData=MyDocument.querySelectorAll(".player-card-description.gray-900");
        let birthDate = recievedData[1].textContent;
